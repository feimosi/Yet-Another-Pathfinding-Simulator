...
bool InputCollector::loadDataFromImage(int rows=50)
{
    MapParse parse (dataImage);
    Settings sett ();
    float depth;
    float mapWidth = (dataImage.getSize().x / scale) - 1;
    for (; mapHeight > 0 && rows > 0; mapHeight--, rows--){
        ...
        */
        if (match[3] == "jpg" || match[3] == "png") {
            ...
            if (dataImage.loadFromFile(filePath) == true){
                ...
                mapHeight =  std::min(riverBottom.getHeight(), (int) (dataImage.getSize().y / scale)) - 1;
                return true;
                }
                return false;
        }
