...
void FuzzyControlSystem::Fuzzifier::fuziffy(int front, int left, int right) {
	...
	for (int i = 0; i < 3; i++) {
		fuzzySets[FRONT][i] = membershipFunctions[i](1, frontLow, frontMedium, frontHigh);
		fuzzySets[LEFT][i] = membershipFunctions[i](1, leftLow, leftMedium, leftHigh);
		fuzzySets[RIGHT][i] = membershipFunctions[i](1, rightLow, rightMedium, rightHigh);
	}
}